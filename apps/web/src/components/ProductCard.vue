...
<template>
  <article class="overflow-hidden rounded-[1.25rem] bg-white shadow-[0_8px_30px_rgba(76,44,25,.08)]">
    <img class="block h-[250px] w-full object-cover max-md:h-[290px]" :src="product.image" :alt="product.name" />
    <div class="p-[1.2rem]">
      <div class="flex items-start justify-between gap-3">
        <h3 class="m-0 text-[1.35rem] leading-[1.1]">{{ product.name }}</h3>
        <strong class="whitespace-nowrap text-[#b85131]">{{ formatEuro(product.priceCents) }}</strong>
      </div>
      <p class="min-h-10 leading-[1.35] text-[#695149]">{{ product.description }}</p>
