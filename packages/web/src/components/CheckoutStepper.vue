...
defineProps<{ currentStep: 1 | 2 | 3 | 4 }>();
const emit = defineEmits<{ goToStep: [step: 1 | 2 | 3] }>();

const steps = ["Panier", "Coordonnées", "Récapitulatif", "Confirmation"];
</script>
...
<template>
  <ol class="mb-8 flex items-start font-sans text-xs font-bold uppercase tracking-[.08em] text-[#80685d] sm:mb-10">
    ...
    >
      <button
        v-if="currentStep < 4 && index + 1 < currentStep"
        type="button"
        ...
        @click="emit('goToStep', (index + 1) as 1 | 2 | 3)"
      >
        ...
        <span class="ml-2 hidden underline-offset-4 group-hover:underline lg:block">{{ step }}</span>
      </button>
      <template v-else>
        <span
        ...
        >{{ index + 1 }}</span>
        <span class="ml-2 hidden lg:block">{{ step }}</span>
      </template>
      <span v-if="index < steps.length - 1" class="mx-2 h-px min-w-2 flex-1 bg-[#dfcec3] sm:mx-3" />
    </li>
  </ol>
</template>
