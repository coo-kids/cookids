...
const selectedDeliveryLocation = computed(() =>
  locations.find((location) => location.id === deliveryLocation.value),
);
const hasFixedDeliveryDates = computed(
  () => (selectedDeliveryLocation.value?.fixedDeliveryDates.length ?? 0) > 0,
);
...
<template>
  <form class="mt-6" @submit.prevent="submitDetails">
    <section aria-labelledby="customer-details-title">
      ...
      /></label>
      <label class="my-4 block font-sans text-[.92rem] font-bold"
        >Nom <span class="text-[#80685d] font-normal">(facultatif)</span
        ><input
          class="mt-[.4rem] block w-full rounded-[.55rem] border border-[#d9c6b8] bg-white p-3 font-serif font-normal"
      ...
      /></label>
      <label class="my-4 block font-sans text-[.92rem] font-bold"
        >Téléphone <span class="text-[#80685d] font-normal">(facultatif)</span
        ><input
          class="mt-[.4rem] block w-full rounded-[.55rem] border border-[#d9c6b8] bg-white p-3 font-serif font-normal"
    ...
    <section
      class="mt-10 border-t border-[#eadace] pt-8"
      ...
      <label class="my-4 block font-sans text-[.92rem] font-bold"
        ...
        <span v-if="hasFixedDeliveryDates" class="relative mt-[.4rem] block">
          ...
          >
            ...
            <option
              v-for="date in availableDeliveryDates"
              :key="date"
              :value="date"
            >
              {{ date }}
          ...
          <span
            ...
            >Aucune date de livraison disponible pour ce lieu.</span
          >
        </span>
        <input
          v-else
    ...
    <div class="mt-8 flex flex-col gap-3 sm:flex-row">
      <AppButton
        ...
        variant="neutral"
        @click="emit('back')"
        >Revenir au panier</AppButton
      >
      <AppButton class="flex-1" type="submit"
        >Valider les coordonnées</AppButton
      >
    </div>
  </form>
</template>
