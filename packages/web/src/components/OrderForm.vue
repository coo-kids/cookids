...
// Use the customer's local calendar date, not UTC, including month/year rollover.
...
const availableDeliveryDates = computed(() =>
  (selectedDeliveryLocation.value?.fixedDeliveryDates ?? []).filter((date) => date >= minimumDeliveryDate)
);

watchEffect(() => {
  if (targetDeliveryDate.value && (
    targetDeliveryDate.value < minimumDeliveryDate ||
    (hasFixedDeliveryDates.value && !availableDeliveryDates.value.includes(targetDeliveryDate.value))
  )) {
...
function submitDetails(): void {
  if (hasFixedDeliveryDates.value && !availableDeliveryDates.value.includes(targetDeliveryDate.value)) return;
  if (targetDeliveryDate.value && targetDeliveryDate.value < minimumDeliveryDate) return;
    ...
    <section class="mt-10 border-t border-[#eadace] pt-8" aria-labelledby="delivery-details-title">
      ...
      <label class="my-4 block font-sans text-[.92rem] font-bold">Date de livraison souhaitée
        <span v-if="hasFixedDeliveryDates" class="relative mt-[.4rem] block">
          <select class="block w-full appearance-none rounded-[.55rem] border border-[#d9c6b8] bg-white p-3 pr-11 font-serif font-normal text-[#3f332d]" v-model="targetDeliveryDate" :disabled="availableDeliveryDates.length === 0" required>
            <option value="" disabled>Choisir une date</option>
            <option v-for="date in availableDeliveryDates" :key="date" :value="date">{{ date }}</option>
          </select>
          ...
          <span v-if="availableDeliveryDates.length === 0" class="mt-2 block font-normal text-[#80685d]">Aucune date de livraison disponible pour ce lieu.</span>
        </span>
        <input v-else class="mt-[.4rem] block w-full rounded-[.55rem] border border-[#d9c6b8] bg-white p-3 font-serif font-normal" v-model="targetDeliveryDate" type="date" :min="minimumDeliveryDate" @click="openDatePicker" />
      </label>
