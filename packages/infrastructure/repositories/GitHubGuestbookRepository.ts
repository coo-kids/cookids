  ...
  private getConnection() {
    ...
    if (!token)
      throw new ServiceUnavailable(
        "Le livre d’or est temporairement indisponible.",
      );
    const settings = constant<GuestbookSettings>("guestbook");
    if (!settings)
      throw new ServiceUnavailable("Configuration du livre d’or indisponible.");
  ...
  async listPublished(): Promise<GuestbookEntry[]> {
    try {
      ...
      const issues = await client.paginate(client.rest.issues.listForRepo, {
        ...
        labels: settings.publishedLabel,
        state: "all",
        sort: "created",
        direction: "desc",
        per_page: 100,
      ...
      return issues.flatMap((issue) => {
        if (
          issue.pull_request ||
          issue.type?.node_id !== settings.issueType ||
          !issue.labels.some((label) =>
            typeof label === "string"
              ? label === settings.publishedLabel
              : label.name === settings.publishedLabel,
          )
        ) {
          return [];
        }
      ...
      console.error("GitHubGuestbookRepository.listPublished failed", {
      ...
      throw new ServiceUnavailable(
        "Les avis sont temporairement indisponibles. Réessayez plus tard.",
      );
    }
  }
  ...
  async save(entry: GuestbookEntry): Promise<void> {
    try {
      ...
      const issue = await client.rest.issues.create({
        owner: settings.owner,
        repo: settings.repository,
        title: `Avis de ${entry.author}`,
        body: formatGuestbookBody(entry),
      });
      await client.graphql(setIssueType, {
        issueId: issue.data.node_id,
        issueTypeId: settings.issueType,
      });
    } catch {
      throw new ServiceUnavailable(
        "Votre avis n’a pas pu être enregistré. Réessayez plus tard.",
      );
    }
  }
}
