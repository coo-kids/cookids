...
describe("defineFetchHandler", () => {
  it("rejette les méthodes non autorisées sans appeler le handler", async () => {
    ...
    const handler = defineFetchHandler({ method: "POST", handler: callback });

    const response = await handler(new Request("https://cookids.test/api/orders"));
  ...
  it("sérialise les erreurs inattendues du callback", async () => {
    ...
    const response = await handler(new Request("https://cookids.test/api/orders", { method: "POST" }));
