...
- Les DTO utilisent `@tsed/schema` + `@tsed/ajv` pour la validation. Employer `@tsed/json-mapper` pour désérialiser les entrées et sérialiser les réponses ; toujours passer `useAlias: false` à `deserialize()`.
...
Tout commit qui implémente un ticket GitHub doit inclure le footer suivant dans son message, sur une ligne distincte : `Closes: #<issueNumber>`.
